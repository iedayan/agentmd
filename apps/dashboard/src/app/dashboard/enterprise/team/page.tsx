...
export default function TeamPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!notificationsRes.ok) throw new Error(notificationsRes.error ?? "Failed to load notifications.");
      ...
      <div className="p-6 md:p-10 space-y-10">
        <div className="relative overflow-hidden rounded-[2rem] border border-primary/20 glass-card p-10 bg-gradient-to-br from-background via-background/90 to-primary/10 shadow-glow/5">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-primary/80">Identity & Access</p>
              <h1 className="text-3xl font-black tracking-tight sm:text-5xl text-gradient mt-4">Team Governance</h1>
              <p className="mt-4 max-w-2xl text-base font-medium text-muted-foreground leading-relaxed">
                Manage member roles, route human-in-the-loop approvals, and assign repository ownership for granular resource isolation.
              </p>
            </div>
            ...
            <Card className="glass-card bg-background/40 backdrop-blur-md border border-border/40 overflow-hidden shadow-2xl">
              <CardHeader className="p-6 border-b border-border/10 flex flex-row items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <CardTitle className="text-lg font-black text-foreground/90 tracking-tight">Access Directory</CardTitle>
                    <CardDescription className="text-[10px] font-black uppercase tracking-widest opacity-60 mt-1">Manage {members.length} team members</CardDescription>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <div className="h-2 w-2 rounded-full bg-emerald-500 animate-glow-pulse" />
                  <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Vault Sync Active</span>
                </div>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
                {loading ? (
                  <div className="space-y-4">
                    {[1, 2, 3].map(i => <div key={i} className="h-24 rounded-2xl animate-pulse bg-muted/20 border border-border/20" />)}
                  </div>
                ) : members.map((member) => (
                  <div key={member.id} className="bento-card border-luminescent group p-6 transition-all duration-300 hover:border-primary/40">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <p className="text-sm font-black text-foreground/90 tracking-tight">{member.name}</p>
                            {member.roleId === "admin" && <Crown className="h-3 w-3 text-orange-500" />}
                          </div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="space-y-1 shrink-0">
                          <label className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-widest ml-1">Assigned Role</label>
                          <select
                            value={member.roleId}
                    ...
                    <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-border/10 pt-4">
                      <span className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-[0.2em]">Ownership:</span>
                      {member.ownedRepositoryIds.map((repositoryId) => (
                        <Badge key={repositoryId} variant="outline" className="text-[9px] font-black tracking-widest uppercase px-2.5 py-0.5 border-primary/20 bg-primary/5 text-primary">
                          ID:{repositoryId}
                        </Badge>
                      ))}
                      <div className="flex-1 min-w-[200px] flex gap-2">
                        ...
                        />
                        <Button size="sm" variant="ghost" className="h-8 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-primary/10 hover:text-primary transition-all" onClick={() => void updateOwnership(member.id)}>
                          Sync IDs
                        </Button>
                      </div>
                    </div>
            ...
            <Card className="glass-card bg-indigo-500/[0.03] border-indigo-500/20 shadow-2xl overflow-hidden border-beam">
              <CardHeader className="p-6 border-b border-indigo-500/10 bg-indigo-500/5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <CardTitle className="text-lg font-black text-foreground/90 tracking-tight">Approval Queue</CardTitle>
                    <CardDescription className="text-[10px] font-black uppercase tracking-widest opacity-60 mt-1">Pending policy guardrails</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <ApprovalQueue
            ...
            <Card className="glass-card bg-background/40 backdrop-blur-md border border-border/40 overflow-hidden shadow-xl">
              <CardHeader className="p-6 border-b border-border/10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <CardTitle className="text-lg font-black text-foreground/90 tracking-tight">Governance Log</CardTitle>
                    <CardDescription className="text-[10px] font-black uppercase tracking-widest opacity-60 mt-1">Real-time activity stream</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-0">
                <div className="divide-y divide-border/10">
                  {notifications.length === 0 ? (
                    <p className="p-10 text-center text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Zero Events recorded.</p>
                  ) : (
                    notifications.slice(0, 10).map((notification) => (
                      <div key={notification.id} className={cn(
                        ...
                        <div className="flex items-center justify-between gap-3 mb-1">
                          <p className="text-xs font-black text-foreground/80 tracking-tight">{notification.title}</p>
                          <div className="shrink-0 flex items-center gap-2">
                            <span className="text-[8px] font-black text-muted-foreground/40 uppercase font-mono">
                              {new Date(notification.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </span>
                            {!notification.read && <span className="h-1.5 w-1.5 rounded-full bg-primary animate-glow-pulse" />}
                          </div>
                        </div>
                        <p className="text-[10px] font-medium text-muted-foreground leading-relaxed line-clamp-2">{notification.message}</p>
                        {!notification.read && (
              ...
              <div className="p-4 border-t border-border/10 flex justify-center">
                <Button variant="link" className="text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 text-muted-foreground hover:text-primary group">
