...
export default function SSOPage() {
  ...
  if (!config) {
    return (
      <div className="p-10 text-center min-h-[400px] flex flex-col items-center justify-center">
        ...
        <Button onClick={() => void loadData()} variant="outline" className="mt-4">
          Retry Sync
        </Button>
      </div>
    );
  ...
  return (
    <FeatureGate feature="Identity Federation & Compliance" planRequired="enterprise">
        ...
        <div className="relative overflow-hidden rounded-[2rem] border border-primary/20 glass-card p-10 bg-gradient-to-br from-background via-background/90 to-primary/10 shadow-glow/5">
          ...
          <div className="relative z-10">
            ...
            <p className="mt-4 max-w-2xl text-base font-medium text-muted-foreground leading-relaxed">
              Configure enterprise identity protocols and access real-time compliance evidence for
              external audits and stakeholder reviews.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-10 lg:grid-cols-12">
          <div className="lg:col-span-12 grid gap-6 md:grid-cols-4">
            {[
              ...
              {
                label: 'Enforcement',
                value: config.enforceSso ? 'Required' : 'Optional',
                icon: Lock,
                color: 'text-indigo-500',
              },
              {
                label: 'Evidence',
                value: `${compliance.filter((c) => c.status === 'ready').length} Ready`,
                icon: FileCheck,
              ...
              <div
                key={stat.label}
                className="glass-card p-5 border border-border/40 bg-background/40 hover:border-primary/20 transition-all group"
              >
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={cn(
                      'h-8 w-8 rounded-lg flex items-center justify-center bg-muted/20 border border-border/20 group-hover:bg-primary/5 transition-all',
                      stat.color,
                    )}
                  >
                ...
                <p className="text-xl font-black text-foreground/90 tracking-tight">
                  {stat.label === 'SSO Mode' && loading ? '...' : stat.value}
                </p>
              </div>
            ))}
          </div>
          ...
          <div className="lg:col-span-7 space-y-10">
            <Card className="glass-card bg-background/40 backdrop-blur-md border border-border/40 overflow-hidden shadow-2xl">
              ...
              <CardContent className="p-8 space-y-8">
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setConfig((prev) =>
                          prev
                            ? { ...prev, provider: event.target.value as SsoConfig['provider'] }
                            : null,
                        )
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        setConfig((prev) =>
                          prev ? { ...prev, entityId: event.target.value } : null,
                        )
                      }
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest ml-1">
                    SSO Entrypoint (ACS URL)
                  </label>
                  <Input
                    value={config.ssoUrl}
                    onChange={(event) =>
                      setConfig((prev) => (prev ? { ...prev, ssoUrl: event.target.value } : null))
                    }
                ...
                <div className="grid gap-4 md:grid-cols-3 pt-6">
                  <Button
                    ...
                    className={cn(
                      ...
                      config.enabled &&
                        'shadow-glow bg-primary text-primary-foreground hover:bg-primary/90',
                    )}
                  >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setConfig((prev) => (prev ? { ...prev, enforceSso: !prev.enforceSso } : null))
                    }
                    className={cn(
                      'rounded-xl h-12 btn-tactile font-black text-[10px] uppercase tracking-widest',
                      config.enforceSso &&
                ...
                <div className="pt-4 flex items-center justify-between text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest group cursor-help">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      Fingerprint:{' '}
                      {loading ? '...' : (config.updatedAt || 'unverified').slice(0, 8)}
                    </span>
                  </div>
                    ...
                    <div key={item.id} className="p-6 hover:bg-primary/[0.01] transition-all group">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <Badge
                            variant="outline"
                            className="text-[9px] font-black tracking-widest px-2 py-0 uppercase border-border/40 bg-muted/20"
                          >
                            {item.version}
                          </Badge>
                        </div>
          ...
          <div className="lg:col-span-5 space-y-10">
            <Card className="glass-card bg-background/40 backdrop-blur-md border border-border/40 overflow-hidden shadow-2xl">
                ...
                <div className="divide-y divide-border/10">
                  {loading && compliance.length === 0 ? (
                    ...
                    compliance.map((artifact) => (
                      <div
                        key={artifact.id}
                        className="p-6 hover:bg-primary/[0.01] transition-all group flex items-start justify-between gap-4"
                      >
                        <div className="space-y-1 flex-1">
                          <div className="flex items-center gap-2">
                            <Badge
                              variant="outline"
                              className="text-[9px] font-black tracking-widest px-2 py-0 uppercase border-border/40 text-muted-foreground/60"
                            >
                              {artifact.framework}
                            </Badge>
                            <span className="text-sm font-black text-foreground/80 tracking-tight">
              ...
              <div className="p-5 bg-emerald-500/5 border-t border-emerald-500/10">
                <Button
                  variant="ghost"
                  className="w-full justify-between group hover:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400"
                >
