...
${policies
  ...
  const togglePrGate = async (id: string, currentEnforcement: boolean) => {
    const updated = policies.map(p => p.id === id ? { ...p, enforcePrGate: !currentEnforcement } : p);
      ...
      <div className="p-6 md:p-10 space-y-10">
        ...
        <div className="relative overflow-hidden rounded-[2rem] border border-primary/20 glass-card p-10 bg-gradient-to-br from-background via-background/90 to-primary/10 shadow-glow/5">
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-primary/80">Governance Engine</p>
            <h1 className="text-3xl font-black tracking-tight sm:text-5xl text-gradient mt-4">Policy-as-Code</h1>
            <p className="mt-4 max-w-2xl text-base font-medium text-muted-foreground leading-relaxed">
              Enforce security guardrails and required GitHub status gates. High-stakes agentic actions are filtered through runtime YAML definitions.
            </p>
          </div>
        </div>
              ...
              <CardHeader className="p-6 border-b border-border/10">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <CardTitle className="text-lg font-black text-foreground/90 tracking-tight">Active Policy Set</CardTitle>
                      <CardDescription className="text-[10px] font-black uppercase tracking-widest opacity-60 mt-1">Runtime enforcement rules</CardDescription>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="h-2 w-2 rounded-full bg-emerald-500 animate-glow-pulse" />
                    <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Enforcement On</span>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
                {loading ? (
                  <div className="space-y-4">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="h-24 animate-pulse rounded-2xl border border-border/20 bg-muted/20" />
                    ))}
                  </div>
                ) : (
                  policies.map((policy) => (
                    <div key={policy.id} className="bento-card border-luminescent group p-6 transition-all duration-300 hover:border-primary/40">
                      <div className="flex items-center justify-between gap-6">
                        <div className="flex-1">
                          <div className="flex items-center gap-3">
                            <p className="text-sm font-black text-foreground/90 tracking-tight group-hover:text-primary transition-colors">{policy.name}</p>
                            <Badge variant="outline" className="text-[10px] opacity-40 font-black">ID: {policy.id}</Badge>
                          </div>
                          <div className="mt-4 flex flex-wrap gap-2">
                            {policy.requireApprovalForPatterns.map((pattern) => (
                              <Badge key={`approve-${pattern}`} variant="warning" className="px-3 py-1 text-[9px] font-black tracking-widest uppercase">
                                <History className="h-3 w-3 mr-1" />
                                APPROVE: {pattern}
                              </Badge>
                            ))}
                            {policy.blockPatterns.map((pattern) => (
                              <Badge key={`block-${pattern}`} variant="destructive" className="px-3 py-1 text-[9px] font-black tracking-widest uppercase">
                          ...
                          <div className="mt-4 flex items-center gap-4">
                            ...
                            >
                              <GitPullRequest className={cn("h-3 w-3", policy.enforcePrGate && "animate-pulse")} />
            ...
            <Card className="glass-card bg-background/40 backdrop-blur-md border border-border/40 overflow-hidden shadow-2xl">
              <CardHeader className="p-6 border-b border-border/10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <CardTitle className="text-lg font-black text-foreground/90 tracking-tight">GitHub Status Gates</CardTitle>
                    <CardDescription className="text-[10px] font-black uppercase tracking-widest opacity-60 mt-1">Required merge & run checks</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-6 space-y-6">
                {gates.map((gate) => (
                  <div key={gate.repositoryId} className="bento-card bg-muted/20 border-border/40 p-6">
                    <div className="flex items-center gap-3 mb-6">
                      <Dna className="h-4 w-4 text-primary opacity-60" />
                      <p className="text-sm font-black text-foreground/90 tracking-tight">{gate.repositoryName}</p>
                    </div>
                    <div className="grid gap-4 md:grid-cols-2">
                      {gate.requiredChecks.map((check) => {
                        const status = gate.checks[check] ?? "missing";
                        return (
                          <div key={check} className="rounded-2xl border border-border/40 bg-background/40 p-4 transition-all hover:bg-background/60">
                            <div className="flex items-center justify-between gap-4 mb-4">
                              <span className="text-[11px] font-black text-foreground tracking-tight truncate">{check}</span>
                              <Badge
                                variant={
                                  status === "success"
                                    ? "success"
                              ...
                              {(["success", "pending", "failed"] as const).map((nextStatus) => (
                                <Button
                                  ...
                                  className={cn(
                                    ...
                                    status === nextStatus && "border-primary/40 bg-primary/10 text-primary"
                                  )}
          ...
          <div className="lg:col-span-4 space-y-10">
            <Card className="glass-card bg-indigo-500/[0.03] border-indigo-500/20 shadow-2xl overflow-hidden relative border-beam">
              <CardHeader className="p-6 space-y-2 border-b border-indigo-500/10 bg-indigo-500/5">
                ...
                <CardTitle className="text-xl font-black text-foreground/90 tracking-tight">Diagnostic Lab</CardTitle>
                <CardDescription className="text-xs font-medium text-muted-foreground">Test execution preflights against runtime policies.</CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-6">
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest ml-1">Repository Identifier</label>
                    <Input
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest ml-1">Target Agent</label>
                    <Input
            ...
            <Card className="glass-card bg-muted/20 border-border/40 overflow-hidden shadow-xl group">
              <CardHeader className="p-6 border-b border-border/10">
                <div className="flex items-center gap-3">
                  ...
                  <CardTitle className="text-sm font-black text-foreground/90 tracking-tight uppercase tracking-widest">Policy Blueprint (YAML)</CardTitle>
                </div>
              </CardHeader>
                  ...
                  <pre className="p-6 text-[10px] font-black font-mono leading-relaxed text-muted-foreground bg-muted/30 overflow-x-auto selection:bg-primary/20 max-h-[400px]">
                    {policyYamlPreview.split('\n').map((line, i) => (
                      <div key={i} className="flex gap-4">
                        <span className="w-4 shrink-0 opacity-20 select-none text-right">{i + 1}</span>
                        <span className={cn(
                          line.trim().startsWith('-') ? "text-primary" :
                            line.includes('enabled: true') ? "text-emerald-500/70" :
                              line.includes('enabled: false') ? "text-red-500/70" : ""
                        )}>{line}</span>
                      </div>
                    ))}
                  </pre>
