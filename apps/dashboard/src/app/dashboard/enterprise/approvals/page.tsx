...
import { Activity, History, FileText, Lock, MessageSquare, Filter } from 'lucide-react';
...
export default function ApprovalsPage() {
  ...
  return (
    <div className="p-6 md:p-10 space-y-10">
      <div className="relative overflow-hidden rounded-[2rem] border border-indigo-500/20 glass-card p-10 bg-gradient-to-br from-background via-background/90 to-indigo-500/10 shadow-glow/10 border-beam">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-4">
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-500/80">
              Governance Terminal
            </p>
            <h1 className="text-3xl font-black tracking-tight sm:text-5xl text-gradient from-foreground to-indigo-500/60 transition-all">
              Approval Workflows
            </h1>
            <p className="max-w-xl text-base font-medium text-muted-foreground leading-relaxed">
              Human-in-the-loop validation for high-stakes agent operations. Enforce mandatory
              multi-sig approvals for sensitive resource mutations.
            </p>
          </div>
      ...
      <div className="flex items-center justify-between pb-2 border-b border-border/10">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2 group cursor-pointer">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-foreground group-hover:text-indigo-500 transition-all">
              Pending Task ({approvals.filter((a) => a.status === 'pending').length})
            </span>
          </div>
        ...
        <Button
          variant="ghost"
          size="sm"
          className="h-8 rounded-lg gap-2 text-[9px] font-black uppercase tracking-widest hover:bg-muted/40"
        >
      ...
      <ApprovalQueue approvals={approvals} onDecide={handleDecide} loading={loading} />
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-xs font-medium text-muted-foreground mt-1">
                  Enterprise customers can define specialized approval topologies via Slack or
                  Microsoft Teams.
                </p>
              </div>
            </div>
            <Button
              size="sm"
              variant="outline"
              className="rounded-xl font-black text-[10px] uppercase h-10 px-6 border-orange-500/20 text-orange-600 hover:bg-orange-500/5"
            >
