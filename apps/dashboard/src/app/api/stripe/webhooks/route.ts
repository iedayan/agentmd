...
async function handleCheckoutSessionCompleted(
  ...
  if (!resolvedUserId) {
    ...
    if (!email) return;
    const res = await pool.query(`SELECT id FROM users WHERE LOWER(email) = LOWER($1) LIMIT 1`, [
      email,
    ]);
...
export async function POST(req: NextRequest) {
  ...
  try {
    event = new Stripe(stripeSecretKey).webhooks.constructEvent(body, signature, webhookSecret);
  } catch (err) {
