...
describe("GET /api/analytics/contracts", () => {
  it("returns contract analytics grouped by type", async () => {
    ...
    const res = await GET(
      new Request("http://localhost/api/analytics/contracts?days=30") as unknown as import("next/server").NextRequest
    );
    ...
    expect(json.byType.some((item: { contractType: string }) => item.contractType === "bugfix")).toBe(
      true
    );
