...
`),
    ...
    const res = await GET(createRequest('http://localhost/api/badge/score?repo=owner/repo'));
    expect(res.status).toBe(200);
  ...
  it('returns score 0 when repo fetch fails', async () => {
    ...
    const res = await GET(createRequest('http://localhost/api/badge/score?repo=owner/nonexistent'));
    expect(res.status).toBe(200);
    const text = await res.text();
    expect(text).toContain('0/100');
  });
