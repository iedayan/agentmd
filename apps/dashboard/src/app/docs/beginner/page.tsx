...
export default function BeginnerDocsPage() {
  return (
    <div>
      ...
      <CodeBlock>{`npx @agentmd-dev/cli init`}</CodeBlock>
      <p className="text-sm text-muted-foreground mt-2">
        Or with pnpm: <code>pnpm add -D @agentmd-dev/cli && pnpm exec agentmd init</code>
      </p>

      <h2>Step 2: Validate before running</h2>
      <p>Make sure your instructions are safe and structured:</p>
      <CodeBlock>{`npx @agentmd-dev/cli validate
npx @agentmd-dev/cli score`}</CodeBlock>

      <h2>Step 3: Preview, then run</h2>
      <p>Dry-run first. Then run only test commands:</p>
      <CodeBlock>{`npx @agentmd-dev/cli run . --dry-run
npx @agentmd-dev/cli run . test`}</CodeBlock>
      ...
      <CodeBlock>{`npx @agentmd-dev/cli run . --use-shell`}</CodeBlock>
