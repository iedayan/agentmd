...
export function RecentActivityFeed({ executions, className }: RecentActivityFeedProps) {
  ...
  return (
    <div
      className={cn(
        'flex flex-col h-full glass-card border-luminescent overflow-hidden',
        className,
      )}
    >
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-primary/70 font-bold truncate">
                      CMD_{exe.id.split('-')[0].toUpperCase()}
                    </span>
                  ...
                  <div className="mt-1 text-white/90 truncate">
                    ...
                    {exe.trigger === 'push'
                      ? `git push detected on ${exe.repositoryId}`
                      : `manual trigger: run ${exe.id}`}
                  </div>
                  {activeId === exe.id && (
                    <div className="mt-2 pt-2 border-t border-white/10 space-y-1 animate-in fade-in slide-in-from-top-1 duration-200">
                      <div className="flex items-center gap-2 text-white/60">
                        ...
                        <span>
                          Status:{' '}
                          <span className={cn('font-bold', getStatusColor(exe.status))}>
                            {exe.status}
                          </span>
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-white/60">
                        ...
                        <span>
                          Duration:{' '}
                          {exe.completedAt
                            ? `${Math.round((new Date(exe.completedAt).getTime() - new Date(exe.startedAt).getTime()) / 1000)}s`
                            : 'In Progress'}
                        </span>
                      </div>
                    </div>
      ...
      <div className="px-4 py-2 border-t border-primary/10 bg-primary/5 flex items-center gap-4 overflow-hidden">
        <div className="flex items-center gap-2 whitespace-nowrap">
          ...
          <span className="text-[9px] text-primary/50 font-bold uppercase tracking-widest">
            System Health
          </span>
        </div>
