...
const PROJECT_TYPES = [
    { id: "node", name: "Node.js (pnpm/npm)", icon: Zap, commands: "## Build\n`pnpm install`\n`pnpm run build`\n\n## Test\n`pnpm test`" },
    { id: "python", name: "Python (pip/poetry)", icon: Zap, commands: "## Setup\n`pip install -r requirements.txt`\n\n## Test\n`pytest`" },
    { id: "rust", name: "Rust (cargo)", icon: Zap, commands: "## Build\n`cargo build`\n\n## Test\n`cargo test`" },
    { id: "go", name: "Go", icon: Zap, commands: "## Build\n`go build ./...`\n\n## Test\n`go test ./...`" },
];
...
export function AgentSetupWizard() {
    ...
    const selectedTemplate = (GOVERNANCE_TEMPLATES as GovernanceTemplate[]).find(t => t.id === projectData.templateId) || BASELINE_SECURITY_TEMPLATE;
        ...
        <div className="max-w-4xl mx-auto py-10 px-4">
            <div className="mb-8 flex items-center justify-between">
                ...
                <div className="text-right">
                    <p className="text-xs text-muted-foreground font-medium uppercase tracking-widest">Step {step} of 3</p>
                    <p className="text-sm font-semibold">{step === 1 ? "Project Identity" : step === 2 ? "Governance Policy" : "Final Review"}</p>
                </div>
            </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {PROJECT_TYPES.map((t) => (
                                    ...
                                    >
                                        <t.icon className={cn("h-5 w-5", projectData.type === t.id ? "text-primary" : "text-muted-foreground")} />
                    ...
                    <CardContent className="space-y-4">
                        {(GOVERNANCE_TEMPLATES as GovernanceTemplate[]).map((t) => (
                            ...
                            >
                                <div className={cn(
                                    "mt-1 p-2 rounded-lg",
                                    projectData.templateId === t.id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                                )}>
                                    {t.id === "strict-compliance" ? <Lock className="h-5 w-5" /> : <Shield className="h-5 w-5" />}
                                </div>
            ...
            {step === 3 && (
                <Card className="glass-card border-primary/20">
                    <CardHeader>
                        ...
                        <CardDescription>Your agent is ready. Place this file in your repository root.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        ...
                        <div className="mt-8 p-4 rounded-lg bg-primary/5 border border-primary/20 flex gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                    You can also run <code className="bg-muted px-1 rounded">agentmd init</code> in your terminal to achieve the same result with auto-detection.
                                </p>
                            </div>
                        </div>
                    </CardContent>
