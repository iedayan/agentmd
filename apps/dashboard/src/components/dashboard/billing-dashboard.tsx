        ...
        return (
            <div className="p-8 space-y-8 animate-pulse">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {[1, 2, 3].map(i => <div key={i} className="h-64 rounded-3xl bg-muted/20" />)}
                </div>
            </div>
        );
    }
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div>
                        <div className="flex items-center gap-3">
                            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-primary/80">Account & Billing</p>
                            <Badge className="bg-primary/20 text-primary border-primary/20 text-[9px] font-black px-2 py-0 border-luminescent">
                                {currentPlan.name} PLAN
                            </Badge>
                        </div>
                        <h1 className="text-3xl font-black tracking-tight sm:text-5xl text-gradient mt-4">Subscription Overview</h1>
                    ...
                    <div className="flex gap-4">
                        <Button variant="outline" className="rounded-2xl border-border/40 font-black text-[11px] px-6 h-12 btn-tactile">
                        ...
                        <Card className="glass-card bg-background/40 backdrop-blur-md border border-border/40 overflow-hidden shadow-2xl group">
                            <CardHeader className="p-6 pb-2">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className="text-[11px] font-black text-primary uppercase tracking-widest">Compute</span>
                                </div>
                                <CardTitle className="text-2xl font-black tracking-tight">Execution Minutes</CardTitle>
                            </CardHeader>
                            <CardContent className="p-6 pt-2">
                                <div className="flex justify-between items-end mb-4">
                                    <span className="text-3xl font-black">{status?.usage.executionMinutes} <span className="text-sm text-muted-foreground font-medium">/ {currentPlan.executionMinutes === Infinity ? "∞" : currentPlan.executionMinutes}</span></span>
                                    <span className="text-[10px] font-black text-muted-foreground/60 uppercase">Resetting in 12 days</span>
                                </div>
                                <Progress value={(status?.usage.executionMinutes || 0) / (currentPlan.executionMinutes === Infinity ? 1 : currentPlan.executionMinutes) * 100} className="h-2 bg-muted/30" />
                            </CardContent>
                        </Card>
                        ...
                        <Card className="glass-card bg-background/40 backdrop-blur-md border border-border/40 overflow-hidden shadow-2xl group">
                            <CardHeader className="p-6 pb-2">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className="text-[11px] font-black text-indigo-500 uppercase tracking-widest">Assets</span>
                                </div>
                                <CardTitle className="text-2xl font-black tracking-tight">Repositories</CardTitle>
                            </CardHeader>
                            <CardContent className="p-6 pt-2">
                                <div className="flex justify-between items-end mb-4">
                                    <span className="text-3xl font-black">{status?.usage.repositories} <span className="text-sm text-muted-foreground font-medium">/ {currentPlan.repositories === Infinity ? "∞" : currentPlan.repositories}</span></span>
                                    <span className="text-[10px] font-black text-muted-foreground/60 uppercase">Manual Sync Active</span>
                                </div>
                                <Progress value={(status?.usage.repositories || 0) / (currentPlan.repositories === Infinity ? 1 : currentPlan.repositories) * 100} className="h-2 bg-indigo-500/20" />
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <Card className="glass-card bg-background/40 backdrop-blur-md border border-border/40 overflow-hidden shadow-2xl">
                        <CardHeader className="p-8 border-b border-border/10">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <CardTitle className="text-xl font-black text-foreground/90 tracking-tight">Plan Selection</CardTitle>
                                    <CardDescription className="text-xs font-medium text-muted-foreground/60">Compare tiers and unlock enterprise capabilities.</CardDescription>
                                </div>
                            </div>
                        </CardHeader>
                                    ...
                                    return (
                                        <div key={pid} className={cn(
                                            ...
                                            <div className="flex items-center justify-between mb-6">
                                                <span className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-[0.2em]">{p.name}</span>
                                                {isCurrent && <Badge variant="success" className="text-[8px] font-black uppercase border-luminescent">Active</Badge>}
                                            </div>
                                            <div className="mb-4">
                                                <span className="text-4xl font-black tracking-tight">${p.price}</span>
                                                <span className="text-muted-foreground/60 text-xs font-medium ml-1">/ mo</span>
                                            </div>
                                            <p className="text-[11px] text-muted-foreground/80 mb-8 leading-relaxed min-h-[40px]">
                                                {pid === 'free' ? "For individuals and small personal projects." :
                                                    pid === 'pro' ? "For growing teams that need performance and visibility." :
                                                        "The complete governance platform for security-first organizations."}
                                            </p>

                                            <div className="space-y-4 mb-10 flex-1">
                                                {[
                                                    { text: `${p.repositories === Infinity ? 'Unlimited' : p.repositories} Repositories`, ok: true },
                                                    { text: `${p.executionMinutes === Infinity ? 'Unlimited' : p.executionMinutes} Min / mo`, ok: true },
                                                    ...
                                                    <div key={idx} className={cn("flex items-center gap-3 text-[11px]", feat.ok ? "text-foreground/80 font-bold" : "text-muted-foreground/30 font-medium")}>
                                                        <CheckCircle2 className={cn("h-3.5 w-3.5", feat.ok ? "text-primary" : "text-border")} />
                                                        {feat.text}
                                                    </div>
                                                ))}
                                            </div>
                                            ...
                                            <Button
                                                variant={isCurrent ? "outline" : pid === 'enterprise' ? "default" : "secondary"}
                                                ...
                                                className={cn(
                                                    ...
                                                    pid === 'enterprise' && !isCurrent && "bg-primary shadow-glow hover:bg-primary/90"
                                                )}
                                            >
                ...
                <div className="lg:col-span-4 space-y-8">
                    <Card className="glass-card bg-indigo-500/[0.03] border-indigo-500/20 shadow-2xl overflow-hidden relative border-beam border-luminescent">
                        <CardHeader className="p-8">
                            ...
                            <CardTitle className="text-xl font-black text-foreground/90 tracking-tight">Enterprise Guard</CardTitle>
                        ...
                        <CardContent className="p-8 pt-0 space-y-4">
                            <div className="flex items-center justify-between p-4 rounded-2xl bg-background/40 border border-border/10 hover:border-primary/20 transition-all cursor-pointer">
                                <div className="flex flex-col gap-0.5">
                                    ...
                                    <span className="text-[10px] text-muted-foreground font-medium underline">Learn more</span>
                                </div>
                            ...
                            <div className="flex items-center justify-between p-4 rounded-2xl bg-background/40 border border-border/10 hover:border-primary/20 transition-all cursor-pointer">
                                <div className="flex flex-col gap-0.5">
                                    ...
                                    <span className="text-[10px] text-muted-foreground font-medium underline">Top up</span>
                                </div>
