    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <div className="bento-card border-luminescent group bg-gradient-to-br from-primary/[0.05] to-transparent p-1">
          ...
          <CardContent>
            ...
            <p className="text-[10px] text-muted-foreground/60 font-black mt-2 tracking-widest">
              LATENCY AGGREGATE
            </p>
            <div className="mt-5 h-1.5 w-full bg-muted/40 rounded-full overflow-hidden border border-border/10 shadow-inner">
              <div
                ...
                style={{
                  width: `${loading ? 0 : Math.min(100, ((data?.impact.avgExecutionSeconds ?? 0) / 120) * 100)}%`,
                }}
              />
            </div>
          </CardContent>
        ...
        <div className="bento-card border-luminescent group bg-gradient-to-br from-emerald-500/[0.05] to-transparent p-1">
          ...
          <CardContent>
            <div className="text-4xl font-black text-emerald-500 text-primary-glow">
              {loading ? '…' : `${data?.impact.automationHoursSaved ?? 0}`}{' '}
              <span className="text-sm font-black opacity-40 ml-1">HRS</span>
            </div>
            <p className="text-[10px] text-muted-foreground/60 font-black mt-2 tracking-widest uppercase">
              Efficiency Surplus
            </p>
          </CardContent>
        </div>
        <div className="bento-card border-luminescent bg-primary/[0.03] p-1 border-beam">
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] text-primary/80">
              ROI Multiple
            </CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-[10px] text-primary/60 font-black mt-2 tracking-widest uppercase">
              Capital Efficiency
            </p>
          </CardContent>
        </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Link href="/docs/roi-methodology">
              <span className="text-[10px] font-bold uppercase tracking-widest text-primary hover:underline">
                Methodology ↗
              </span>
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {(contracts?.byType ?? []).slice(0, 6).map((item) => (
              <div
                ...
                className="rounded-2xl border border-border/40 p-4 bg-background/50 group hover:border-primary/20 transition-colors"
              >
                <div className="flex items-center justify-between mb-2">
                  <p className="font-bold text-sm text-foreground/80">{item.contractType}</p>
                  <p className="text-[10px] font-bold bg-muted px-2 py-0.5 rounded-full">
                    {item.total} INSIGHTS
                  </p>
                </div>
                <div className="flex items-center gap-4">
                  <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                    <div className="h-full bg-primary" style={{ width: `${item.passRate}%` }} />
                  </div>
            ...
            {!loading && (contracts?.byType.length ?? 0) === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">
                No contract checks recorded yet.
              </p>
            ) : null}
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Execution volume and failures by day</CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        point.failedExecutions > 0
                          ? 'bg-gradient-to-t from-red-500/40 to-red-400/60'
                          : 'bg-gradient-to-t from-primary/60 to-primary-bright/80',
                      )}
                      style={{
          ...
          <div className="mt-3 grid gap-3 text-sm md:grid-cols-2">
            <div className="rounded-md border p-3">
              ...
              <p className="text-muted-foreground">
                {loading
                  ? '…'
                  : `${data?.kpis.failurePreventionEstimate ?? 0} potential failures prevented`}
              </p>
            </div>
            <div className="rounded-md border p-3">
