...
export function FeatureGate({ children, feature, planRequired, className }: FeatureGateProps) {
  ...
  const hasAccess =
    (planRequired === 'pro' &&
      (currentPlan === 'pro' || currentPlan === 'enterprise' || hasProFeatures(currentPlan))) ||
  ...
  return (
    <div
      className={cn(
        'relative min-h-[400px] overflow-hidden rounded-[2rem] border border-border/40 group',
        className,
      )}
    >
        ...
        <div className="max-w-md w-full glass-card border-primary/20 bg-background/80 p-10 text-center shadow-2xl relative overflow-hidden border-luminescent border-beam">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-2xl font-black tracking-tight mb-4">
              {feature} is an {planRequired === 'enterprise' ? 'Enterprise' : 'Pro'} Feature
            </h2>
            <p className="text-sm font-medium text-muted-foreground leading-relaxed mb-8">
              Enable advanced governance, centralized management, and priority support by upgrading
              your workspace.
            </p>

            <div className="flex flex-col gap-3">
              <Button
                className="w-full rounded-[1.25rem] h-12 font-black text-[11px] uppercase tracking-widest shadow-glow btn-tactile"
                asChild
              >
              ...
              <Button
                variant="ghost"
                className="w-full rounded-[1.25rem] h-12 font-black text-[11px] uppercase tracking-widest text-muted-foreground hover:text-foreground"
                asChild
              >
