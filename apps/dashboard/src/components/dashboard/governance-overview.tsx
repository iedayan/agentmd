...
type Approval = { id: string; status: string; repositoryName: string; reason: string };
...
export function GovernanceOverview() {
  ...
  useEffect(() => {
    ...
    fetch("/api/workflows/approvals", { cache: "no-store" })
      .then((r) => r.json())
      .then((d: { ok?: boolean; approvals?: Approval[] }) => {
        if (!cancelled && d.ok && Array.isArray(d.approvals)) {
      ...
      <CardContent className="space-y-4">
        ...
        {pendingCount > 0 && (
          <p className="text-xs text-muted-foreground">
            {approvals.find((a) => a.status === "pending")?.repositoryName ?? "Repos"} need your decision
          </p>
        )}
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="ghost" size="sm" asChild className="text-xs">
            <Link href="/dashboard/enterprise/policies">
              Policies
            </Link>
          </Button>
        </div>
