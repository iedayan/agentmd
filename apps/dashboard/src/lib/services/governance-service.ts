...
export const governanceService = {
  ...
  async getNotifications(): Promise<{
    ...
    const body = await res.json();
    return res.ok
      ? { ok: true, notifications: body.notifications }
  ...
  async runPreflight(input: {
    ...
    requestedBy: string;
    agentId: string;
  }): Promise<{ ok: boolean; error?: string; code?: string; details?: { approvalId?: string } }> {
    ...
    return res.ok
      ? { ok: true }
      : { ok: false, error: body.error, code: body.code, details: body.details };
  },
};
