...
export const enterpriseService = {
    ...
    async saveSsoConfig(config: SsoConfig): Promise<{ ok: boolean; sso?: SsoConfig; error?: string }> {
     ...
     */
    async getRbacData(): Promise<{ ok: boolean; members?: Member[]; roles?: Role[]; error?: string }> {
        const res = await fetch("/api/enterprise/rbac", { cache: "no-store" });
        const body = await res.json();
        return res.ok ? { ok: true, members: body.members, roles: body.roles } : { ok: false, error: body.error };
    },

    async updateMemberRole(memberId: string, roleId: string): Promise<{ ok: boolean; error?: string }> {
    ...
    async updateMemberOwnership(memberId: string, ownedRepositoryIds: string[]): Promise<{ ok: boolean; error?: string }> {
        const res = await fetch("/api/enterprise/rbac", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
