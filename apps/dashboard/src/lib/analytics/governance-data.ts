...
function loadState() {
  ...
  if (persisted.governanceMetrics) {
    governanceMetrics = {
      ...
      contractValidationEvents: Array.isArray(
...
export function recordContractValidationResult(input: {
  ...
  if (governanceMetrics.contractValidationEvents.length > 1000) {
    governanceMetrics.contractValidationEvents = governanceMetrics.contractValidationEvents.slice(-1000);
  }
  persistState();
}
...
export function evaluateExecutionPreflight(input: {
  ...
  | { allowed: true }
  | { allowed: false; code: string; reason: string; approvalId?: string } {
  const repository =
    input.repository ??
  ...
  if (owners.length > 0) {
    const isOwner = owners.some(
      (owner) =>
        owner.id.toLowerCase() === requestIdentity ||
        owner.email.toLowerCase() === requestIdentity
    );
    ...
    const isPrivileged = requesterMember?.roleId === "admin" || requesterMember?.roleId === "approver";
  ...
  return {
    ...
    webhookSuccessRate:
      governanceMetrics.webhookReceived > 0
        ? Math.round((governanceMetrics.webhookProcessed / governanceMetrics.webhookReceived) * 1000) / 10
        : 100,
