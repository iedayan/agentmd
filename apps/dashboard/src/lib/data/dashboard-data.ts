...
const marketplaceAgents: AgentListing[] = [
  ...
  },
  {
    ...
    slug: "react-template",
    description:
      "Scaffold React + TypeScript + Vite projects with AGENTS.md pre-configured.",
...
export function listAuditLogs(limit = 100): AuditLog[] {
  return auditLogs.slice().sort((a, b) => Date.parse(b.timestamp) - Date.parse(a.timestamp)).slice(0, limit);
}
