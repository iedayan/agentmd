 ...
 */
export function toOtelTrace(ctx) {
  const traceId = ctx.executionId.replace(/-/g, '').slice(0, 32).padEnd(32, '0');
  const rootSpanId = traceId.slice(0, 16);
  const startNs = BigInt(new Date(ctx.startedAt).getTime() * 1000000);
  const endNs = ctx.completedAt
    ? BigInt(new Date(ctx.completedAt).getTime() * 1000000)
  ...
  if (ctx.steps?.length) {
    ...
    for (const step of ctx.steps) {
      ...
      const stepEndNs = stepStartNs + BigInt(stepDurationMs * 1000000);
      spans.push({
        traceId,
        spanId: stepSpanId,
