 ...
 */
export async function validateAgentsMd(parsed, options) {
  const errors = [];
  const warnings = [];
  const suggestions = [];
 ...
 */
export async function computeAgentReadinessScore(parsed) {
  let score = 0;
  const max = 100;
...
function validateFrontmatterSchema(fm) {
  ...
  const resources = ['pull_requests', 'issues', 'contents'];
  ...
  if (fm.output_contract !== undefined) {
    ...
    const required = ['format', 'schema', 'quality_gates', 'artifacts', 'exit_criteria'];
...
function checkDirectiveConflicts(directives) {
  const warnings = [];
