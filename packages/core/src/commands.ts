...
function buildSectionMap(sections: AgentsMdSection[]): Map<number, string> {
...
function buildSectionContentMap(sections: AgentsMdSection[]): Map<number, string> {
  const map = new Map<number, string>();
  const visit = (s: AgentsMdSection, content: string) => {
    for (let i = s.lineStart; i <= s.lineEnd; i++) {
      map.set(i, content);
    }
...
function inferCommandType(command: string): CommandType {
  ...
  for (const [type, keywords] of Object.entries(TYPE_KEYWORDS)) {
    ...
    for (const k of keywords) {
      ...
      const score = { type: type as CommandType, keywordLen: k.length, atSuffix };
      if (
        !best ||
