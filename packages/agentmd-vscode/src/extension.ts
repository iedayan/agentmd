...
export function activate(context: ExtensionContext): void {
  ...
  client.start().then(() => {
    ...
    const updateStatusBar = async () => {
      ...
      const isAgentsMd =
        doc &&
        (doc.uri.path.endsWith("AGENTS.md") || doc.uri.path.endsWith(".agents.md"));
    ...
    context.subscriptions.push(
      commands.registerCommand("agentmd.validate", async () => {
        ...
        if (!doc || (!doc.uri.path.endsWith("AGENTS.md") && !doc.uri.path.endsWith(".agents.md"))) {
          window.showInformationMessage(
            "Open an AGENTS.md file to validate."
          );
          return;
        }
        window.showInformationMessage(
      ...
      commands.registerCommand("agentmd.showScore", async () => {
        ...
        if (!doc || (!doc.uri.path.endsWith("AGENTS.md") && !doc.uri.path.endsWith(".agents.md"))) {
          window.showInformationMessage(
            "Open an AGENTS.md file to see the score."
          );
          return;
        }
        try {
          ...
          if (result?.score != null) {
            window.showInformationMessage(
              `AgentMD Score: ${result.score}/100`
            );
          } else {
