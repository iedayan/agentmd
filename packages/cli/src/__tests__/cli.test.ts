...
describe("agentmd cli", () => {
  ...
  it("supports check --contract for required output_contract", () => {
    ...
    try {
      ...
      expect(result.status).toBe(1);
      const out = JSON.parse(result.stdout);
      expect(out.errors.some((e: { code: string }) => e.code === "MISSING_OUTPUT_CONTRACT")).toBe(true);
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }
  });
...
`,
      ...
      const result = runCli(["check", tempDir, "--contract", "--output", join(tempDir, "agent-output.json"), "--json"]);
      expect(result.status).toBe(0);
      const out = JSON.parse(result.stdout);
