...
export async function listAgents(options: { category?: string; certified?: boolean }) {
  ...
  const res = await fetch(`https://api.agentmd.online/marketplace/agents?${params}`, {
...
export async function executeAgent(agentId: string, repositoryId: string, apiKey: string) {
