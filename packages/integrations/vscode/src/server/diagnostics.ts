...
const RULE_DOC_BASE = 'https://docs.agentmd.dev/rules';
...
export async function getDiagnostics(
  ...
  for (const { line } of emptyBlocks) {
    diagnostics.push(createDiagnostic(line - 1, 0, 'AMD004', 'Empty command block detected', 'error'));
  }

  // AMD005: No frontmatter
  if (parsed.raw.trim() && !parsed.frontmatter && !content.trimStart().startsWith('---')) {
    diagnostics.push(createDiagnostic(0, 0, 'AMD005', 'Missing frontmatter metadata', 'warning'));
  }

  // AMD007: Absolute path in command
  for (const cmd of parsed.commands) {
    if (ABSOLUTE_PATH_REGEX.test(cmd.command)) {
      diagnostics.push(createDiagnostic(cmd.line - 1, 0, 'AMD007', 'Absolute path detected in command', 'warning'));
    }
  }
